...
def _finished_tasks(keyword):
    ...
    for task_type in task_types:
        tasks = task_type.objects.filter(systems__name__contains=keyword, active=True)
        for task in tasks:
...
if __name__ == "__main__":
    try:
        ...
        print "Please provide a keyword that should be part of the Task system names"
        keyword = "Round 4"

    for task in _finished_tasks(keyword):
        task_name = task.task_name.replace("/",".")
        filename = directory+"/"+task_name+"."+type(task).__name__+".results.xml"
        sys.stderr.write("Writing {}".format(filename))
