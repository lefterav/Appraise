...
optionParser.add_option("-d", "--idfile", dest="idfile", help="list o ids matching the source sentences")
...
for (custom_id, line) in zip(fids, fp):
    #s = d.sourcesentence_set.create(text=l.strip())
    s = models.SourceSentence(text=line.strip())
    s.document = d
